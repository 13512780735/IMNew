    ...
    private void initGroupMemberList() {
        SealUserInfoManager.getInstance().getGroupMembers(groupId, new SealUserInfoManager.ResultCallback<List<GroupMember>>() {
            ...
            public void onSuccess(List<GroupMember> groupMembers) {
                Logger.d("groupMembers:-->"+groupMembers);
    ...
    private void fillSourceDataListWithFriendsInfo() {
        SealUserInfoManager.getInstance().getFriends(new SealUserInfoManager.ResultCallback<List<Friend>>() {
            ...
            public void onSuccess(List<Friend> friendList) {
                Logger.d("friendList:-->"+friendList);

                if (mListView != null) {
                    if (friendList != null && friendList.size() > 0) {
                        for (Friend friend : friendList) {
                            Logger.d("FrienduserId:-->"+friend.getUserId());
                        ...
                        if (isAddGroupMember) {
                            for (GroupMember groupMember : addGroupMemberList) {
                                ...
                                Logger.d("groupMemberId:-->"+groupMember.getUserId());
                                for (int i = 0; i < data_list.size(); i++) {
        ...
        if (deleteGroupMemberList != null && deleteGroupMemberList.size() > 0) {
            for (GroupMember deleteMember : deleteGroupMemberList) {
                ...
                data_list.add(new Friend(deleteMember.getUserId(),
                        ...
                        deleteMember.getDisplayName() //TODO displayName 需要处理 暂为 null
                ));
            }
            fillSourceDataList();
            updateAdapter();
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
            ...
            AsyncTaskManager.getInstance(mContext).request(REQUSERINFO, new OnDataListener() {
                ...
                public Object doInBackground(int requestCode, String parameter) throws HttpException {
                    return new SealAction(mContext).getFriendInfoByID(SharedPreferencesUtil.getString(mContext,"userId",""),adapterList.get(position).getUserId());
                }
                ...
                public void onSuccess(int requestCode, Object result) {
                    if (result != null) {
                        ...
                        if(TextUtils.isEmpty(getFriendInfoByIDResponse.getResultData().getRemark())){
                            viewHolder.tvTitle.setText(getFriendInfoByIDResponse.getResultData().getUserNickName());
                        }else {
                            viewHolder.tvTitle.setText(getFriendInfoByIDResponse.getResultData().getRemark());
                        }
                    }
                }
    ...
    public Object doInBackground(int requestCode, String id) throws HttpException {
        switch (requestCode) {
            case ADD_GROUP_MEMBER:
                return action.addGroupMember(SharedPreferencesUtil.getString(mContext,"userId",""),groupName,groupId, startDisList);
            case DELETE_GROUP_MEMBER:
                return action.deleGroupMember(groupId, startDisList, SharedPreferencesUtil.getString(mContext,"userId",""));
        }
            ...
            switch (requestCode) {
                case ADD_GROUP_MEMBER:
                    ...
                    if (res.isSuccess() == true) {
                        ...
                        BroadcastManager.getInstance(mContext).sendBroadcast(SealAppContext.UPDATE_GROUP_MEMBER);
                        finish();
                    }else {
                        LoadDialog.dismiss(mContext);
                        NToast.shortToast(mContext, res.getMessage());
        ...
        switch (v.getId()) {
            case R.id.text_right:
                if (mCBFlag != null && sourceDataList != null && sourceDataList.size() > 0) {
                    ...
                    if(mFriend!=null){
                    for(int j=0;j<createGroupList.size();j++){
                        if(createGroupList.get(j).getUserId().equals(mFriend.getUserId())){
                           createGroupList.remove(j);
                           // createGroupList.add(mFriend);
                        }
...
//                            }
                    ...
                    if (isConversationActivityStartDiscussion) {
                        ...
                        CommonDialog dialog = new CommonDialog.Builder()
                                .setContentMessage(getString(R.string.remove_group_members))
