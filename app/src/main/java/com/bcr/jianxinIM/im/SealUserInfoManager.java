...
import com.bcr.jianxinIM.im.db.BlackListDao;
...
import com.bcr.jianxinIM.im.db.GroupMemberDao;
import com.bcr.jianxinIM.im.db.Groups;
import com.bcr.jianxinIM.im.db.GroupsDao;
import com.bcr.jianxinIM.im.db.UserInfoBean;
...
public class SealUserInfoManager implements OnDataListener {
    ...
    public SealUserInfoManager(Context context) {
        ...
        userId=sp.getString(SealConst.SEALTALK_USERID,"");
     ...
     * @param userId1
     */
    public void getAllUserInfo(String userId1) {
        userId=userId1;
    ...
    private boolean fetchFriends(String userId) throws HttpException {
        //Logger.d("userId1:-->"+SharedPreferencesUtil.getString(mContext,"userId",""));
        UserRelationshipResponse userRelationshipResponse;
        try {
            userRelationshipResponse = action.getAllUserRelationship(userId);
        } catch (JSONException e) {
            NLog.d(TAG, "fetchFriends occurs JSONException e=" + e.toString());
            return true;
        }
        if (userRelationshipResponse != null && userRelationshipResponse.isSuccess() == true) {
            List<UserRelationshipResponse.ResultDataBean> list = userRelationshipResponse.getResultData();
            Logger.d("好友列表1：--》"+list);
    ...
    public List<Friend> pullFriends() throws HttpException {
       // Logger.d("userId2:-->"+ SharedPreferencesUtil.getString(mContext,"userId",""));
    ...
    private boolean fetchGroups(String userId) throws HttpException {
        GetGroupResponse groupResponse;
        try {
            groupResponse = action.getGroups(SharedPreferencesUtil.getString(mContext,"userId",""));
        } catch (JSONException e) {
            NLog.d(TAG, "fetchGroups occurs JSONException e=" + e.toString());
            return true;
        }
        if (groupResponse != null && groupResponse.isSuccess() == true) {
            List<GetGroupResponse.ResultDataBean> groupsList = groupResponse.getResultData();
            ...
            public void run() {
                try {
                    ...
                    if (!hasGetGroups()) {
                        ...
                        if (groupInfoResponse != null && groupInfoResponse.isSuccess() == true) {
                            GetGroupInfoResponse.ResultDataBean groupInfo = groupInfoResponse.getResultData();
                            if (groupInfo != null) {
    ...
    private List<Groups> pullGroups() throws HttpException {
        ...
        if (groupResponse != null && groupResponse.isSuccess() == true) {
            List<GetGroupResponse.ResultDataBean> list = groupResponse.getResultData();
            if (list != null && list.size() > 0) {
                syncDeleteGroups();
    ...
    private boolean fetchGroupMembers() throws HttpException {
        ...
        if (mGroupsList != null && mGroupsList.size() > 0) {
            ...
            for (Groups group : mGroupsList) {
                ...
                if (groupMemberResponse != null && groupMemberResponse.isSuccess() == true) {
                    fetchGroupCount++;
                    List<GetGroupMemberResponse.ResultDataBean> list = groupMemberResponse.getResultData();
                    if (list != null && list.size() > 0) {
    ...
    private boolean fetchBlackList() throws HttpException {
        ...
        try {
            blackListResponse = action.getBlackList(SharedPreferencesUtil.getString(mContext,"userId",""));
        } catch (JSONException e) {
            NLog.d(TAG, "fetchBlackList occurs JSONException e=" + e.toString());
            return true;
        }
        if (blackListResponse != null && blackListResponse.isSuccess() == true) {
            List<GetBlackListResponse.ResultDataBean> blackList = blackListResponse.getResultData();
    ...
    private List<BlackList> pullBlackList() throws HttpException {
        ...
        if (blackListResponse != null && blackListResponse.isSuccess() == true) {
            List<GetBlackListResponse.ResultDataBean> responseBlackList = blackListResponse.getResultData();
...
//                        mFriendDao.insertOrReplace(friend);
     ...
     */
    public List<Friend> addFriends(final List<UserRelationshipResponse.ResultDataBean> list) {
        Logger.d("Friend111:-->"+list);
        if (list != null && list.size() > 0) {
            List<Friend> friendsList = new ArrayList<>();
            for (UserRelationshipResponse.ResultDataBean resultDataBean : list) {
                if (resultDataBean.getStatus() == 20) {
                    Friend friend = new Friend(
                            resultDataBean.getUserId(),
                            resultDataBean.getUserNickName(),
                            Uri.parse(resultDataBean.getUserPic()),
                            resultDataBean.getRemark(),
                            null, null, null, null,
                            CharacterParser.getInstance().getSpelling(resultDataBean.getUserNickName()),
                            CharacterParser.getInstance().getSpelling(resultDataBean.getRemark()));
                    if (friend.getPortraitUri() == null || TextUtils.isEmpty(friend.getPortraitUri().toString())) {
     ...
     */
    private List<Groups> addGroups(final List<GetGroupResponse.ResultDataBean> list) {
        if (list != null && list.size() > 0) {
            mGroupsList = new ArrayList<>();
            for (GetGroupResponse.ResultDataBean groups : list) {
                String portrait = groups.getPicture();
                if (TextUtils.isEmpty(portrait)) {
                    portrait = RongGenerate.generateDefaultAvatar(groups.getNickName(), groups.getGroupId());
                }
                mGroupsList.add(new Groups(groups.getGroupId(),
                        groups.getNickName(),
                        portrait,
     ...
     */
    private List<GroupMember> addGroupMembers(final List<GetGroupMemberResponse.ResultDataBean> list, final String groupID) {
        if (list != null && list.size() > 0) {
     ...
     */
    public List<BlackList> addBlackList(List<GetBlackListResponse.ResultDataBean> responseList) {
        if (responseList != null && responseList.size() > 0) {
            List<BlackList> blackList = new ArrayList<>();
            for (GetBlackListResponse.ResultDataBean black : responseList) {
                blackList.add(new BlackList(black.getUserId(),
                        null,
                        null
                ));
            }
        ...
        mWorkHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (!doingGetAllUserInfo && !hasGetFriends()) {
                    ...
                    try {
                        fetchFriends(SharedPreferencesUtil.getString(mContext,"userId",""));
                    } catch (HttpException e) {
                        e.printStackTrace();
                    }
     ...
     * @param callback 群组列表的回调
     */
    public void getGroups1(final ResultCallback<List<Groups>> callback) {
        mWorkHandler.post(new Runnable() {
            @Override
            public void run() {
                List<Groups> groupsList;
...
//                }
    ...
    public List<GroupMember> getGroupMembers1(String groupID) {
        if (TextUtils.isEmpty(groupID)) {
            return null;
        } else {
            if (mGroupMemberDao != null) {
    ...
    public void getGroupMembers1(final String groupID, final ResultCallback<List<GroupMember>> callback) {
        if (TextUtils.isEmpty(groupID)) {
            if (callback != null) {
                callback.onError(null);
            }
    ...
    private List<GroupMember> setCreatedToTop(List<GetGroupMemberResponse.ResultDataBean> groupMember, String groupID) {
        List<GroupMember> newList = new ArrayList<>();
        GroupMember created = null;
        for (GetGroupMemberResponse.ResultDataBean group : groupMember) {
            ...
            GroupMember newMember = new GroupMember(groupID,
                    group.getUserId(),
                    group.getUserNickName(),
                    Uri.parse(group.getUserPic()),
                    group.getRemarkTitle(),
                    CharacterParser.getInstance().getSpelling(group.getUserNickName()),
                    CharacterParser.getInstance().getSpelling(group.getRemarkTitle()),
                    groupName,
    ...
    public String getPortraitUri(GetGroupInfoResponse groupInfoResponse) {
        if (groupInfoResponse.getResultData() != null) {
            Groups groups = new Groups(groupInfoResponse.getResultData().getId(),
                    groupInfoResponse.getResultData().getName(),
                    groupInfoResponse.getResultData().getPortraitUri());
