 ...
 */
package com.bcr.jianxinIM.activity.base;
...
import com.bcr.jianxinIM.activity.base_util.StringUtils;
import com.bcr.jianxinIM.activity.base_util.SystemUtils;
 ...
 */
public abstract class BaseFragment extends Fragment implements BaseInterface, OnDataListener {
	...
	public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
		...
		initData();
		addListeners();
		action = new SealAction(getActivity());
		if(rootView!=null)
			return rootView;
	...
	public void onFailure(int requestCode, int state, Object result) {
		switch (state) {
			...
			case AsyncTaskManager.HTTP_NULL_CODE:
				NToast.shortToast(getActivity(), "当前网络不可用");
				break;

			// 网络有问题给出提示
			case AsyncTaskManager.HTTP_ERROR_CODE:
				NToast.shortToast(getActivity(), "网络问题请稍后重试");
				break;
